...
/*	1. In der Funktion wird die verwendete Vokabelliste/datei vom User ausgew�hlt
 * 	2. Das ausgew�hlte file wird auf Kategorienmarker �berpr�ft, also ob es eine thematische Unterteilung der Vokabeln gibt
 * 	3. Die Sprache am Kopfende der Datei wird ausgelesen und in der vokabelstruktur "sprache" gespeichert, welche der Funktion �bergeben wurde
 * 	4. Entsprechend dieses Ergebnisses wird das Dokument char f�r char ausgelesen und in dynamischen listen abgesspeichert
 * 		4.1 Exsistieren Kategorien wird eine Kategorienliste erstellt, jede Kategorie enth�lt ihre entssprechenden Vokabelpaare als Listen
 * 		4.2 Gibt es keine Kategorien wird direkt eine Gesamt-Kategorie, mit allen Vokabeln erstellt und zur�ckgegeben
 * 	5. Sollte Fall 3.1 aussgef�hrt worden sein, wird die Kategorienliste an eine Gesamt-Kategorie angef�gt, welche alle vokabel enth�lt.
 */
struct kategorie *liste_einlesen(struct vokabel * sprache){
	...
	char needle4[] = ".csv";
	int Aufzaehlung = 0;																					//Variable zum Hochz�hlen bei Ausgabe der Dateinamen beginnend bei 1
	...
	//Ausgeben der m�glichen Vokabellisten
	printf("Es wird in demselben Ordner, in welchem dieses Programm liegt, nach Vokabellisten gesucht:\n");	//Ausgabe die den Nutzer �ber die Suche nach Vokabellisten informiert
		setbuf(stdout, NULL);
		dir = opendir(".");																					//Mit der Funktion opendir wird ein Verzeichnis ge�ffnet, in diesem Fall das Verzeichnis, in dem die Anwendung liegt -> (".") und dir zugewiesen
		mark2:
		...
		if(dir){																							//Wenn dir ungleich 0 wird in die if-Abfrage verzweigt
			while (0 != (entity = readdir(dir))) {															//Schleife zum Auslesen aller Dateinamen, solange bis keine Eintr�ge mehr im Verzeichnis sind
				if(strcmp(entity->d_name, ".") == 0 ||														//Wird eventuell sp�ter ben�tigt, wenn Verzeichnis weiter verzeigt ist
                   strcmp(entity->d_name, "..") == 0){														//Wird eventuell sp�ter ben�tigt, wenn Verzeichnis weiter verzeigt ist
                    continue;																				//
                    }																						//

                if(((strstr(entity->d_name, needle) != 0) || strstr(entity->d_name, needle4) != 0)&&		//If-Verzweiung, wenn der Dateiname ein .txt ocer .csv enth�lt
                   (strstr(entity->d_name, needle2) == 0) &&												//und nicht den String Usernames enth�lt, da dies keine Vokabelliste ist
				   (strstr(entity->d_name, needle3) == 0)){													//und nicht den String Highscore enth�lt, da dies ebenfalls keine Vokabelliste ist
                    ...
                    if(Flagge_Entity == false)																//If-Verzweigung zum �berpr�fen ob eine struct entity_name besteht
                    {
                    ...
                    else																					//Else-Verzweigung
                    {
                    	...
                    	strcpy(ende_entity_name_ptr->next->name, entity->d_name);							//Der Name der n�chsten Datei wird in die neu erstellte Struct geschrieben
        ...
        }																									//Nach dem Ablauf der Funktion wird das zu Beginn ge�ffnete Verzeichnis wieder geschlossen
		}
    //Abfrage nach der zu �ffnenden Datei

    printf("Bitte wählen Sie die Liste aus, welche abgefragt werden soll, \n"									//Ausgabe einer Informationsnachricht f�r den Nutzer
    		"indem Sie die entsprechende Nummer eingeben und mit der Enter-Taste bestätigen:");			//Ausgabe einer Informationsnachricht f�r den Nutzer
	...
	if(dummy_zeichen == '\r')																				//auch auf anderen OS (haben evtl mehr Zeichen)
	{
    	...
    	if(listenname >= Aufzaehlung || listenname <= 0 || isalpha(listenname) != 0)						//Eingabe�berpr�fung der Nutzereingabe
			{
				printf("\nUngültige Eingabe! Wählen Sie bitte eine Kategorie mit der "						//Ausgabe einer Informationsnachricht f�r den Nutzer
				...
				dir = opendir(".");																			//Das Verzeichnis wird ge�ffnet
	...
	datei_vorlage_ptr = fopen(entity_name_ptr->name, "r");													//Die ausgew�hlte Datei lesend �ffnen und Anfangsadresse in File-Pointer datei_vorlage_ptr schreiben

	if(datei_vorlage_ptr == NULL)																			//If-Abfrage ob der Pointer datei_vorlage_ptr auf NULL zeigt, denn dann konnte das File nicht ge�ffnet werden
	{
		printf("\nDie Datei konnte leider nicht geöffnet werden.\n");										//Ausgabe einer Informationsnachricht f�r den Nutzer
	...
	printf("\nEs wurde die Datei \"%s\" ausgewählt.\n", entity_name_ptr->name);								//Ausgabe einer Informationsnachricht f�r den Nutzer
	...
	//�berpr�fen des Files auf mindestens eine Kategorie
	...
	while(c != EOF)																							//While-Schleife zum zeichenweisen Auslesen des Files solange c nicht End-Of-File beinhaltet
	{
		...
		if(c == '!')																						//Die If-Abfrage �berpr�ft das ausgelesene Zeichen auf ein "!", was gleichbedeutend mit dem Anfang einer Kategorie ist
		{
	...
	fseek(datei_vorlage_ptr, 0, SEEK_SET);																	//Die Funktion fseek() setzt den Pointer datei_vorlage_ptr zur�ck auf den Anfang des Files
	...
	while(c != EOF)																							//While-Schleife zum zeichenweisen Auslesen des Files solange c nicht End-Of-File beinhaltet
	{
		...
		if(c == ';' || c == '\r' || c == '\n'){																//Zeichen die das Ende eines Wortes, oder den Platz dazwischen signalisieren werden abgefangen
			...
			if(Flagge_Sprache1_begonnen == true && Flagge_Sprache2_begonnen == false){						//Das erste Formatierungszeichen nach dem Beginn des ersten Wortes signalisiert dessen Ende
				Flagge_Sprache1_zuende = true;																//Mit dieser Flagge wird der Zugang zu Sprache1 verschlossen und der zu Sprache2 ge�ffnet
				array_position_vokabel = 0;																	//Z�hl Variable muss zur�ck auf 0 gesetzt werden f�r Sprache 2
				continue;
			}
			if(Flagge_Sprache2_begonnen == true){															//Das erste Formatierungszeichen nach dem Beginn des zweiten Wortes signalisiert dessen Ende
				array_position_vokabel = 0;																	//Auch hier muss die Z�hlvariable zur�ckgesetz werden, da diese sp�ter weiter verwendet wird
		...
		if(Flagge_Sprache1_zuende == false){																//Einlesen von Sprache1 beginnt, falls nicht schon geschehen
			Flagge_Sprache1_begonnen = true;																//Das Einlesen von Sprache1 beginnt, das n�chste Formatierungszeichen wird es wieder beenden
			sprache->vokabel_sprache1[array_position_vokabel] = c;											//Das momentane Zeichen aus dem Dokument wird an das Wort Sprache1 angeh�ngt
			array_position_vokabel++;																		//Somit muss die Z�hlvariable weiter gesetzt werden
			continue;
		}																									//Ende des Einlesens von Sprache1
		if(Flagge_Sprache1_zuende == true){																	//Einlesen von Sprache2 beginnt
			Flagge_Sprache2_begonnen = true;																//Das Einlesen von Sprache2 beginnt, das n�chste Formatierungszeichen wird es wieder beenden und die Schleife wird verlassen
			sprache->vokabel_sprache2[array_position_vokabel] = c;											//Das momentane Zeichen aus dem Dokument wird an das Wort Sprache2 angeh�ngt
			array_position_vokabel++;																		//Somit muss die Z�hlvariable weiter gesetzt werden
			continue;
		}
	...
	if(Flagge_Kategorie_mind == true){																		//Die If-Abfrage �berpr�ft ob die Variable Flagge_Kategorie_mind auf true steht

		  while(c != EOF)																					//While-Schleife zum zeichenweisen Auslesen des Files solange c nicht End-Of-File beinhaltet
	      {
			  //N�chstes Zeichen auslesen
	    	  c = fgetc(datei_vorlage_ptr);																	//Mit der Funktion fgetc() wird ein Zeichen aus dem File ausgelesen und in die Variable c geschrieben

	    	  //Zeilenbr�che raustrennen
	    	  if((Flagge_Semikolon == true && c == ';') ||c == '\r' || c == EOF)							//If-Abfrage die als Filter f�r Zeilenumbr�che und doppelte Semikolons wirkt
	    	  {
	    		  continue;																					//Erneuter Beginn der While-Schleife zum zeichenweisen Auslesen des Files solange c nicht End-Of-File beinhaltet
	    	  }
	    	  if(c == ';')																					//If-Abfrage �berpr�ft ob c ein ";" ist
	    	  {
			  ...
			  if(c == '!')																					//If-Abfrage �berpr�ft ob c ein "!" ist
			  {
				  ...
				  if(erste_kategorie_ptr == NULL){															//If-Abfrage �berpr�ft ob der Pointer erste_kategorie_ptr auf NULL zeigt
			  ...
			  if(Flagge_Neue_Kategorie == true)																//If-Abfrage �berpr�ft ob die Variable Flagge_Neue_Kategorie true ist
			  {
			  ...
			  if(Flagge_Vokabelpaar_beschreibbar == false && Flagge_Kategorie_befuellbar == true && c!='\n' && c!=';'){		//
				  ...
				  if(ende_kategorie_ptr->erste_vokabel == NULL){											//If-Abfrage ob der Pointer ende_kategorie_ptr->erste_vokabel auf NULL zeigt
					  ...
					  ende_vokabel_ptr = ende_vokabel_ptr->next_vokabel;									//Der Pointer ende_vokabel_ptr wird auf den Pointer ende_vokabel_ptr->next_vokabel gesetzt (n�chste struct vokabel)
				  }
				  ende_vokabel_ptr->flag = 0;																//�ber den Pointer ende_vokabel_ptr wird die Variable flag auf 0 initialisiert

				  ende_kategorie_ptr->anzahl_in_kategorie++;												//�ber den Pointer ende_kategorie_ptr wird die Variable anzahl_in_kategorie um eins inkrementiert
		...
		while(c != EOF || Flagge_erster_Durchlauf == true)													//While-Schleife zum zeichenweisen Auslesen des Files solange c nicht End-Of-File beinhaltet oder Flagge_erster_Durchlauf true ist
		{
		  ...
		  if(alle_kategorien_ptr == NULL){																	//If-Abfrage ob der Pointer alle_kategorien_ptr auf NULL zeigt
			  ...
			  strcpy(alle_kategorien_ptr->kategorie_name,"Alle Vokabeln");									//Mit der Funktion strcpy() wird der String "Alle Vokabeln" �ber den Pointer alle_kategorien_ptr in die Variable kategorie_name kopiert
		  ...
		  if(Flagge_alleVokabel1_fertig == false){															//If-Abfrage ob die Variable Flagge_alleVokabel1_fertig false ist
			  ...
			  if(Flagge_aktuelles_Vokablpaar == false){														//If-Abfrage ob die Variable Flagge_aktuelles_Vokablpaar false ist
				  ...
				  alle_vokablen_ende_ptr = alle_vokablen_ende_ptr->next_vokabel;							//Der Pointer alle_vokablen_ende_ptr wird auf den Pointer alle_vokablen_ende_ptr->next_vokabel gesetzt (auf die n�chste Vokabel)
				  Flagge_aktuelles_Vokablpaar = true;														//Setzen der Variablen Flagge_aktuelles_Vokablpaar auf true
			  }
		  ...
		  if(Flagge_alleVokabel1_fertig == true){															//If-Abfrage ob die Variable Flagge_alleVokabel1_fertig true ist
			  ...
			  if(c == ';'){																					//If-Abfrage ob c ein ";" ist
				  ...
				  alle_kategorien_ptr->anzahl_in_kategorie++;												//�ber den Pointer alle_kategorien_ptr wird die Variable anzahl_in_kategorie um eins inkrementiert
				  continue;																					//Erneuter Beginn der While-Schleife zum zeichenweisen Auslesen des Files solange c nicht End-Of-File beinhaltet
			  }
		...
		}//Ende While EOF f�rt alle vok
		fclose(datei_vorlage_ptr);
		return(alle_kategorien_ptr);																		//Der Pointer alle_kategorien_ptr wird von der Funktion zur�ck gegeben
	}//Ende else f�r abfrage ob !kategorie



	/*	Eine Gesamt-Kategorie wird erschaffen.
	 * 	Diese enh�lt alle Vokabeln des Dokuments.
	 * 	Diese Kategorie wird vor der ersten in die Kategorieliste eingef�gt
	 */
		if(true){																							//If-Abfrage
			alle_kategorien_ptr = malloc(sizeof(struct kategorie));											//Auf den Pointer alle_kategorien_ptr wird die erste struct kategorie allokiert
			strcpy(alle_kategorien_ptr->kategorie_name,"Alle Vokabeln");									//Mit der Funktion strcpy() wird der String "Alle Vokabeln" �ber den Pointer alle_kategorien_ptr in die Variable kategorie_name kopiert
			...
			alle_kategorien_ptr->anzahl_in_kategorie = erste_kategorie_ptr->anzahl_in_kategorie;			//Anzahl der Vokabeln in der Gesamtkategorie wird im Vorrais auf die der ersten Kategorie erh�ht
			...
			vokabel_cpy_laeufer_ptr = malloc(sizeof(struct vokabel));										//Ein Hilfszeiger um die Vokabeln an das Ende der Gesamtkategorienvokabelliste anzuf�gen
			...
			while(1){																						//Nachdem ein Startpunkt mit der Kopie der ersten Vokabelliste vorhanden ist, k�nnen weitere Kopiervorg�nge nun identisch ablaufen -> Schleife

				while(vokabel_org_laeufer_ptr->next_vokabel != NULL){										//L�uft bis das Ende der Orginal-Vokabelliste erreicht ist
					vokabel_cpy_laeufer_ptr->next_vokabel = malloc(sizeof(struct vokabel));					//Speicher f�r eine neue Vokabel an der Gesamtliste reservieren
					vokabel_cpy_laeufer_ptr = vokabel_cpy_laeufer_ptr->next_vokabel;						//Zeiger auf neuen Speicher setzen
					vokabel_org_laeufer_ptr = vokabel_org_laeufer_ptr->next_vokabel;						//N�chste Vokabel in Orginalliste ausw�hlen


					cpy_vokabel(vokabel_cpy_laeufer_ptr,vokabel_org_laeufer_ptr);							//Kopie von Orginal auf Kopie
				}

				kategorie_laeufer_ptr = kategorie_laeufer_ptr->next_kategorie;								//N�chste Kategorie ausw�hlen
				if(kategorie_laeufer_ptr == NULL)															//Beenden des Kopiervorgangs falls Ende der Kategorienliste erreicht
				{
					break;																					//joa
				}
				vokabel_org_laeufer_ptr = kategorie_laeufer_ptr->erste_vokabel;								//Vokabelliste der neuen Kategorie ausw�hlen
				vokabel_cpy_laeufer_ptr->next_vokabel = malloc(sizeof(struct vokabel));						//Speicherplatz f�r die erste Vokabelkopie aus der neuen Kategorie reservieren
				vokabel_cpy_laeufer_ptr = vokabel_cpy_laeufer_ptr->next_vokabel;							//Zeiger auf neuen Speicher setzen
				...
				alle_kategorien_ptr->anzahl_in_kategorie += kategorie_laeufer_ptr->anzahl_in_kategorie;		//Anzahl der Vokabeln in der Gesamtliste wird im Vorraus um die der neuen Kategorie erh�ht
			}

		}//Ende Gesamtzkategorie erschaffen

	fclose(datei_vorlage_ptr);																				//schlie�en des datenstreams aus der Vokabeldatei
	//closedir(dir);
	return(alle_kategorien_ptr);																			//R�ckgabe der Gesamtkategorieliste
 ...
 */
void cpy_vokabel(struct vokabel * copy,struct vokabel * original){											//Erh�lt die Orginalvokabelstruktur und den Kopiespeicherplatz
	...
	copy->flag = original->flag;																			//Flag(ob abgefragt) kopieren. Eigentlich unn�tig, ab so ist sicher, dass die neue Flag auch geerdet ist
