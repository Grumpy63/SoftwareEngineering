...
scanf("%c", &dummy_zeichen);										//Leeren des Puffers
	...
	//	help_ptr->flag = 1;									  //Flag der Ausgewählten Vokabel setzen
		...
		printf("\nBitte geben sie die Uebersetzung von %s ein: ", help_ptr->vokabel_sprache2);  // Aufforderung zur Eingabe
		...
		if(strcmp(eingabe, help_ptr->vokabel_sprache1) == 0)  //Vergleich der beiden Strings.
			{
			...
			{
				printf("Leider falsch! Korrekte waere gewesen: %s\n", help_ptr->vokabel_sprache1);   //Feedback mit richtiger Übersetzung
			}
