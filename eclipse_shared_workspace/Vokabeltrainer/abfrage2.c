...
char eingabe [256];
	...
	for (int i=0; i<number_abzufragende_voc; i++)												//Schleife für die Abfragezyklen, i ist Laufvariable
	{
			...
			for(int x=0; x < (zufall); x++) 													//Cursor-Zeiger jeweils Vielfaches der Zufallszahl Weitersetzen auf die nächsten Listenelemente
				{
			...
			if (zufall != 0 && zufall != abzufragende_kategorie->anzahl_in_kategorie)																	//zu entfernende Vok befindet sich mittendrin
				{
					...
					help_hinterher_ptr->next_vokabel = cursor_ptr->next_vokabel;				//weiterrouten auf die Restliste
				}
...
printf("\n Auf das zeigt cursor: %s\t %s \n", cursor_ptr->vokabel_sprache1, cursor_ptr->vokabel_sprache2);

struct vokabel* test_ptr =abzufragende_kategorie->erste_vokabel;

printf("\n aktuelle liste nach entfernen: ");
for(int i =0; i<(abzufragende_kategorie->anzahl_in_kategorie); i++ )
{

	printf("\n%s\t%s\n", test_ptr->vokabel_sprache1, test_ptr->vokabel_sprache2);
	test_ptr = test_ptr->next_vokabel;
}
